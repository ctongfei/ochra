...
data_series = [[(x, math.sin(x + i * 0.4) * (16 - i) / 4 + 5) for x in np.linspace(0, 10, 128)] for i in range(12)]

plot = oxp.Chart(
    size=(200, 200),
    x_axis=oxp.ContinuousAxis("x", (0, 10)),
    y_axis=oxp.ContinuousAxis("y", (0, 10)),
    plots=[oxp.LinePlot("sine", data, stroke=oxs.Stroke(color=color)) for data, color in zip(data_series, ios)],
