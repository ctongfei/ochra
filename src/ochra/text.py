...
class Text(Element):
    ...
    def _rotated_actual_bbox(self) -> "Rectangle":
        rect = (
            AxisAlignedRectangle(
                ...
                Point.mk((self.extents.x_bearing + self.extents.width, -self.extents.y_bearing)),
            )
            .rotate(self.angle)
            .translate(self.bottom_left.x, self.bottom_left.y)
        )
        return rect
    ...
    def center(self) -> Point:
        bbox = self.rotated_visual_bbox
        return bbox.center

    @property
    def height(self) -> float:
        return self.rotated_visual_bbox.height

    @property
    def width(self) -> float:
        return self.rotated_visual_bbox.width
    ...
    @classmethod
    def left_centered(cls, text: str, left_center: PointI, angle: float = 0.0, font: Font = Font()) -> "Text":
