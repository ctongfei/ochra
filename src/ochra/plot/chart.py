...
class Chart(Generic[X, Y]):
    def __init__(
        ...
        tr_x = Translation((-X.locate_lower_bound(), 0)) @ Scaling((x_scale, 1))
        tr_y = Translation((0, -Y.locate_lower_bound())) @ Scaling((1, y_scale))
        tr_xy = Translation((-X.locate_lower_bound(), -Y.locate_lower_bound())) @ Scaling((x_scale, y_scale))

        self.background_rectangle = rect_bg
        self.grid = grid.transform(tr_xy)
        self.plot_layer = Group(
            elements=[plot.draw(X, Y) for plot in self.plots],
        ).transform(tr_xy)
        self.extra_layer = Group(elements=self.extra).transform(tr_xy)
        self.border_rectangle = border
        self.legend = legend.translate(
            self.x_size - legend_bbox.width - self.text_padding, self.y_size - legend_bbox.height - self.text_padding
        )
    ...
    def draw(
        ...
        return Group(
            [self.background_rectangle, self.grid, self.plot_layer, self.extra_layer]
    ...
    def _draw_axis(self, axis: Axis, ori: AxisOrientation) -> Group:
            ...
            if axis.major_ticks is None
                ...
                Group(
                    [
                        ...
                        Annotation(
                            tick_point_fn[ori](x)
                            + Vector.unit(tick_angle[ori]) * (self.major_tick_length + self.text_padding),
                            make_label(x),
                        ),
                    ]
                )
        ...
        minor_ticks = (
            []
            if axis.minor_ticks is None
            else [Mark(tick_point_fn[ori](x), marker=minor_tick_marker) for x in axis.minor_ticks]
        )
        axis_label = Annotation(
            axis_line.midpoint
            + Vector.unit(tick_angle[ori]) * (self.major_tick_length + text_size + 2 * self.text_padding),
    ...
    def _draw_grid(self) -> Group:
        ...
        return Group(elements=[*x_lines, *y_lines])
